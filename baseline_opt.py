...
def get_opt_results(data, args, Yvalid, Ytest, Yvalid_precorr=None, Ytest_precorr=None):
    ...
    results['valid_ineq_num_viol_0'] = torch.sum(data.ineq_dist(data.validX, Yvalid , mode='valid') > eps_converge,
                                               dim=1).detach().cpu().numpy()
    results['valid_ineq_num_viol_1'] = torch.sum(data.ineq_dist(data.validX, Yvalid, mode='valid') > 10 * eps_converge,
                                               dim=1).detach().cpu().numpy()
    results['valid_ineq_num_viol_2'] = torch.sum(data.ineq_dist(data.validX, Yvalid, mode='valid') > 100 * eps_converge,
                                               dim=1).detach().cpu().numpy()
    ...
    results['test_ineq_max'] = torch.max(data.ineq_dist(data.testX, Ytest, mode='test'), dim=1)[0].detach().cpu().numpy()
    results['test_ineq_mean'] = torch.mean(data.ineq_dist(data.testX, Ytest, mode='test'), dim=1).detach().cpu().numpy()
    results['test_ineq_num_viol_0'] = torch.sum(data.ineq_dist(data.testX, Ytest, mode='test') > eps_converge,
                                              dim=1).detach().cpu().numpy()
    results['test_ineq_num_viol_1'] = torch.sum(data.ineq_dist(data.testX, Ytest, mode='test') > 10 * eps_converge,
                                              dim=1).detach().cpu().numpy()
    results['test_ineq_num_viol_2'] = torch.sum(data.ineq_dist(data.testX, Ytest, mode='test') > 100 * eps_converge,
                                              dim=1).detach().cpu().numpy()
