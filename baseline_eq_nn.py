...
def complete_f(data, X, Z, args):
    prob_type = args['probType']
    if prob_type in ['simple', 'nonconvex','T2F']:
...
def eval_net(data, X, Ytarget, solver_net, args, prefix, stats):
    ...
    dict_agg(stats, make_prefix('ineq_max'), torch.max(data.ineq_dist(X, Ycorr, mode=prefix), dim=1)[0].detach().cpu().numpy())
    dict_agg(stats, make_prefix('ineq_mean'), torch.mean(data.ineq_dist(X, Ycorr, mode=prefix), dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('ineq_num_viol_0'),
             torch.sum(data.ineq_dist(X, Ycorr, mode=prefix) > eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('ineq_num_viol_1'),
             torch.sum(data.ineq_dist(X, Ycorr, mode=prefix) > 10 * eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('ineq_num_viol_2'),
             torch.sum(data.ineq_dist(X, Ycorr, mode=prefix) > 100 * eps_converge, dim=1).detach().cpu().numpy())
    ...
    dict_agg(stats, make_prefix('raw_ineq_max'), torch.max(data.ineq_dist(X, Y, mode=prefix), dim=1)[0].detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_mean'), torch.mean(data.ineq_dist(X, Y, mode=prefix), dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_0'),
             torch.sum(data.ineq_dist(X, Y, mode=prefix) > eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_1'),
             torch.sum(data.ineq_dist(X, Y, mode=prefix) > 10 * eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_2'),
             torch.sum(data.ineq_dist(X, Y, mode=prefix) > 100 * eps_converge, dim=1).detach().cpu().numpy())
    ...
    if take_grad_steps:
        ...
        with torch.no_grad():
            while (i == 0 or torch.max(torch.abs(data.eq_resid(X, Y_new))) > eps_converge or
                           torch.max(data.ineq_dist(X, Y_new, mode=mode)) > eps_converge) and i < max_steps:
                with torch.no_grad():
                    if partial_corr:
                        Y_step = data.ineq_partial_grad(X, Y_new, mode=mode)
                    else:
                        ineq_step = data.ineq_grad(X, Y_new, mode=mode)
