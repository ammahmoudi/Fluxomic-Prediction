...
def train_net(data, args, save_dir):
    ...
    for i in range(nepochs):
        ...
        for Xtrain in train_loader:
            ...
            train_loss = softloss(data, Xtrain, Yhat_train, args,'train')
...
def eval_net(data, X, solver_net, args, prefix, stats):
    ...
    dict_agg(stats, make_prefix('loss'), softloss(data, X, Y, args,prefix).detach().cpu().numpy())
    ...
    dict_agg(stats, make_prefix('raw_ineq_max'), torch.max(data.ineq_dist(X, Y,prefix), dim=1)[0].detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_mean'), torch.mean(data.ineq_dist(X, Y,prefix), dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_0'),
             torch.sum(data.ineq_dist(X, Y,prefix) > eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_1'),
             torch.sum(data.ineq_dist(X, Y,prefix) > 10 * eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_2'),
             torch.sum(data.ineq_dist(X, Y,prefix) > 100 * eps_converge, dim=1).detach().cpu().numpy())
...
def softloss(data, X, Y, args,mode='full'):
    obj_cost = data.obj_fn(Y)
    ineq_cost = torch.norm(data.ineq_dist(X, Y,mode), dim=1)
...
def grad_steps_all(data, X, Y, args,mode='full'):
